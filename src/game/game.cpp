...
{
    Vec2_F32 dim = font_dim_from_tag_size_string(game_state->font, game_state->cell_size*size, 0.f, 4.f, text);
    Vec2_F32 pos = {
        .x = game_state->center.x - (dim.x/2),
        .y = game_state->center.y - ((dim.y - (game_state->cell_size * adjust_y)) / 2.f)
    };
...
internal void game_init(void)
{
    ...
    game_state->font  = font_tag_from_static_data(&app_font);
    ...
    */
    ...
    game_state->sound.eat       = audio_load_from_memory(game_sound_eat, 0);
    game_state->sound.game_over = audio_load_from_memory(game_over_sound, 0);
    ...
    
    {
        ...
        for (size_t i = 0; i < snake_entity->pos_array.length; i++)
        {
                ...
                game_state->score.current != 0)
            {
                ...
                audio_play_voice_from_sound(game_state->sound.game_over, audio_play_params_default());
            }
        }
        ...
        if (snake_head.x == apple_pos.x && snake_head.y == apple_pos.y)
        {
            ...
            if (game_state->score.current > game_state->score.max)
            {
                game_state->score.max = game_state->score.current;
            }
            audio_play_voice_from_sound(game_state->sound.eat, audio_play_params_default());
        }
